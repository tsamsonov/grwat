...
> We use GeoPackage because it is [OGC-standardized](https://www.geopackage.org), fast, modern and powerful spatial data format that does not include any sidecar files like Esri Shapefile. You can easily convert many spatial data formats to GeoPackage by [QGIS](https://www.qgis.org/) _Export > Save as..._ command in a layer context menu, using `st_write` function from `sf` R package, or via [GDAL](http://www.gdal.org) utils if you prefer command-line processing.
