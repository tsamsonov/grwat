...
wd_cur = getActiveDocumentContext()$path %>% dirname()
setwd(wd_cur)

# Read base data for maps
rivers = st_read('data/rivers.gpkg')
rivers_europe = st_read('data/rivers_europe.gpkg')
lakes = st_read('data/lakes.gpkg')
lakes_europe = st_read('data/lakes_europe.gpkg')
ocean = st_read('data/ocean.gpkg')
