...
There is a dedicated `gr_plot_dis()` function that plot the whole time series in a convenient matrix form. Currently it supports plotting the value, seasons and components of the discharge:
```{r, fig.width=10, fig.height=5}
gr_plot_dis(sep, type = 'value')
gr_plot_dis(sep, type = 'season')
gr_plot_dis(sep, type = 'component')
```

You can also limit the range of years that will be plotted:
```{r, fig.width=10, fig.height=2.5}
gr_plot_dis(sep, years = 1980:1995, type = 'component')
```
...
If we are interested more in the shape and relative dimensions of the hydrographs, and do not want the exact Y axis, then we can proceed with ridgeline plots available at __`ggridges`__ package:
...
Another alternative to the ridgeline plot is the horizon plot, which can be built by __`ggHoriPlot`__ package. Horizon plots are more compact and wllow viewing more hydrogrphs at once:
