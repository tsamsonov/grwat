...
Hydrograph data may contain empty values due to corrupted records or missed observations at specific dates. `gr_fill_gaps()` function allows filling such gaps in the data. This function first expands the date sequence if some dates are missing between the minimum and the maximum date in the table. After expansion all missing observations are filled by linear interpolation between nearest values.
...
> You don't have to name the columns exactly as `D, d, m, y, q`, these short names are used to inform `grwat` functions about the structure of input tables.
...
```{r}
sep = gr_separate(tab, p, alpha = 0.85)
head(sep)
```
...
sep %>% 
  ...
  ggplot() +
  geom_area(aes(Date, Qin), fill = 'steelblue') +
  geom_area(aes(Date, Qbase), fill = 'orangered')
```
