...
#'
#' @param df Hydrograph separation data.frame produced by `gr_separate()`.
...
#'
#' @param df Hydrograph and meteorological variables data.frame produced by `gr_summarize()`.
...
#'
#' @param tests List of tests generated by `test_variables()`
#' @param locale String locale. Currently only English locale is supported. Defaults to 'EN'. 
#'
...
#'
#' @param hdata `data.frame` with two columns: date and discharge
#' @param autocorr Numeric value of the autocorrelation for which the time period will be highlighted.
#'
#' @return ACF plot
#' @export
#'
...
#' Plot discharge value, source and season via raster-based visualization through years
#'
#' @param df Hydrograph separation data.frame produced by `gr_separate()`.
#' @param years Integer vector of years to be plotted.
#' @param type String. Supported options are `'value'`, `'components'`, and `'season'`. Defaults to `'value'`
#' @param locale String locale. Currently only English locale is supported. Defaults to `'EN'`.
#'
...
gr_plot_matrix <- function(df, years = NULL, type = 'value', locale='EN') {
  ...
  if (type == 'value') {
    ...
    tab$Typefake = factor(dplyr::case_when(tab$Type == 0 ~ 'Freshet',
                                           tab$Type == 1 ~ 'Summer',
                                           tab$Type == 2 ~ 'Winter'))
    ...
    tab$Qfake = factor(dplyr::case_when(tab$Qfake == 1 ~ 'Freshet',
                                        tab$Qfake == 2 ~ 'Rain',
                                        tab$Qfake == 3 ~ 'Thaw',
                                        tab$Qfake == 4 ~ 'Ground'), 
                       levels = c('Freshet', 'Rain', 'Thaw', 'Ground'))
...
#' @param rev Boolean. Reverse the palette?
