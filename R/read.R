...
#' 
#' The function reads meteorological variables (temperature and precipitation) from [ERA 5 post-processed daily statistics on single levels](https://cds.climate.copernicus.eu/datasets/derived-era5-single-levels-daily-statistics?tab=download) for using with [gr_join_rean()].
#'
#' @param file_prec Character string path to daily sum precipitation NetCDF file.
#' @param file_temp Character string path to daily mean temperature NetCDF file.
#'
#' @return `list` containing time series, precipitation series, temperature series 
#'   and spatial points (sf)
#' @export
#'
#' @example inst/examples/gr_read_era5_daily.R
#' 
gr_read_era5_daily <- function(file_prec, file_temp){
  ...
  precip = ncdf4::nc_open(file_prec)
  temps = ncdf4::nc_open(file_temp)
  prate = ncdf4::ncvar_get(precip, precip$var$tp) * 86400
  temp = ncdf4::ncvar_get(temps, temps$var$t2m) - 273.15
  
  # Number of days and their values
  ndays.full = temps$dim$valid_time$len
  vals.full = temps$dim$valid_time$vals
  
  # Reconstruct spatial points
  lat = precip$dim$latitude$vals
  lon = precip$dim$longitude$vals
  lonidx = 1:length(lon)
  latidx = 1:length(lat)
  coords = expand.grid(lon, lat)
  idx = expand.grid(lonidx, latidx)
