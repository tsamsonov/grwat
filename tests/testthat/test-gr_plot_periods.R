...
test_that("Period plot has the correct content", {
  ...
  plt = gr_plot_periods(vars, Qygr, year = 1978)
  expect_type(plt, 'list')
  expect_s3_class(plt[[1]], 'ggplot')
  
  plt = gr_plot_periods(vars, Qygr, tests = TRUE)
  expect_type(plt, 'list')
  expect_s3_class(plt[[1]], 'ggplot')
  
  tests = gr_test_vars(vars)
  plt = gr_plot_periods(vars, Qmax, Qygr, tests = tests)
  expect_type(plt, 'list')
  expect_s3_class(plt[[1]], 'ggplot')
  
  plt = gr_plot_periods(vars, Qygr, Qmax, date10w1, Wpol3,
