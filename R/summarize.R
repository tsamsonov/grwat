...
gr_summarize <- function(df) {
  ...
  df %>% 
    ...
    dplyr::summarise(Year1 = min(.data$Year),
              ...
              monmmsummer = .data$Date[which.min(.data$Q / (.data$Type == 1))],
              Qmmwin = min(.data$Q[.data$Type == 2], na.rm = T),
              nommwin = .data$Date[which.min(.data$Q / (.data$Type == 2))],
              ...
              Wgr = sum(.data$Qbase, na.rm = TRUE) * kmyr,
              Wpol2 = sum(.data$Qseas, na.rm = TRUE) * kmyr,
              Wpol1 = .data$Wpol2 + sum((.data$Qseas > 0) * .data$Qbase, na.rm = TRUE) * kmyr,
              Wpol3 = .data$Wpol1 + sum((.data$Qseas > 0) * .data$Qrain, na.rm = TRUE) * kmyr,
              Wpavs2 = sum(.data$Qrain, na.rm = TRUE) * kmyr,
              Wpavs1 = .data$Wpavs2 + sum((.data$Qrain > 0) * .data$Qbase, na.rm = TRUE) * kmyr,
              Wpavthaw2 = sum(.data$Qthaw, na.rm = TRUE) * kmyr,
              Wpavthaw1 = .data$Wpavthaw2 + sum((.data$Qthaw > 0) * .data$Qbase, na.rm = TRUE) * kmyr,
