integer polmon(2), polkol(3), prodspada, nPav, nZam, nWin, FlagGaps, InterpolStep
real grad, grad1, kdQgr1, polgrad(2), Pcr, Tzam, Twin, SignDelta, SignDelta1, PavRate, Tcr1
character infilehydro*19, infilemeteo*24, outfileAllGrWat*26, outfileTotal*22, outfileparameters*28
integer lenin/19/, leninmeteo/24/, lenallGrWat/26/,  lenTotal/22/, lenparameters/28/

write (infilehydro, '(a<lenin>)') 'files\Kam-Tu\in.txt'
write (infilemeteo, '(a<leninmeteo>)') 'files\Kam-Tu\inmeteo.txt'
write (outfileAllGrWat, '(a<lenallGrWat>)') 'files\Kam-Tu\AllGrWat.txt'
write (outfileTotal, '(a<lenTotal>)') 'files\Kam-Tu\Total.txt'
write (outfileparameters, '(a<lenparameters>)') 'files\Kam-Tu\parameters.txt'

! калибруемые параметры

!mome=11 ! месяц, с которого начинается зимняя межень
grad=0.7  ! интенсивность спада/подъема расхода воды за счет грунтовой составляющей, выраженная в долях от расхода предшествующего дня
grad1=2  ! интенсивность спада/подъема расхода воды на спаде половодья за счет грунтовой составляющей, выраженная в долях от расхода предшествующего дня
kdQgr1=300  ! максимально допустимое повышение грунтовой составляющей по сравнению с начальным значением (уже после спада половодья) В процентах!!!
...
polkol(1)=10 ! количество дней с начала половодья с устойчивым увеличением в среднем =установленному градиенту (polgrad(1))
...
polgrad(1)=5 ! значения градиента устойчивого увеличения расхода в начале половодья
polgrad(2)=2 ! во сколько раз средний расход воды за половодье (polkol(3)) гарантированно превышает предшествующий половодью меженный уровень
prodspada=90 ! кол-во дней с даты max расхода деяствия grad, после этого переходит на grad1
nPav=5 !количество дней, за которое анализируется сумма осадков и сумма температур! НЕЧЕТНОЕ ЧИСЛО!
...
Pcr=30 !мм осадков - критическая сумма, которая вызывает значимый паводок
Tcr1=0 !C средняя температура за nPav, <= которой - снег, >  тогда паводки 
Tzam=-10 !С средняя суточная температура события "заморозок"
...
call mainprog(polmon, 2, polkol,3, prodspada, nPav, nZam, nWin, FlagGaps, InterpolStep,grad, grad1, kdQgr1, polgrad,2, Pcr, Tcr1, Tzam, Twin, SignDelta, SignDelta1, PavRate, infilehydro, lenin, infilemeteo, leninmeteo, outfileAllGrWat, lenallGrWat, outfileTotal, lenTotal, outfileparameters, lenparameters)

! подпрограммы:

contains

subroutine mainprog (polmon, ss1, polkol,ss2, prodspada, nPav, nZam, nWin, FlagGaps, InterpolStep,grad, grad1, kdQgr1, polgrad,ss3, Pcr, Tcr1, Tzam, Twin, SignDelta, SignDelta1, PavRate, infilehydro, lenin, infilemeteo, leninmeteo, outfileAllGrWat, lenallGrWat, outfileTotal, lenTotal, outfileparameters, lenparameters)
...
character nme*6(99), nmout*10(99), xr*2, dchar*10, date30s(2)*10,date30w(2)*10, date10s(2)*10,date10w(2)*10, date5s(2)*10,date5w(2)*10, datestart*10, datemax*10, datemaxpavs*10, datemaxpavw*10, datepolend*10, DateTotal*10  ! 99 - максимальное количество файлов
...
integer FlagsPcr(100,1000), FlagsPlusTemp(100,1000), FlagsMinusTemp(100,1000)! FlagsPcr - флаги событий критических осадков,  FlagsPlusTemp - флаги событий оттепелей по температурам, FactMinusTemp - флаги событий замороков по температурам
real Pcr, Psumi, Tcr1, Tcr2, Tsri, SignDelta, z, Qo, afunc, bfunc, SignDelta1, Twin ! Pcr - критическая сумма осадков, которая вызывает значимый паводок, Psumi - сумма осадков за nPav дней, Tcr1 - средняя температура за nPav, <= которой - снег, >  тогда паводки Tcr2 - средняя температура за nPav когда наблюдается талый паводок, SignDelta - значимый пеерпад расходов (для выбора значимых спадов/подъемов при отделении паводков) в долях от максимального расхода воды этого года, z - коэффициент затухания экспаненты, Qo - параметр экспаненциальной функции
...
integer cgaps, InterpolStep, YGapsRow(100), MarkWarm, YearStartGaps, CountPavs(100), CountThaws(100), MarkUpPavs !InterpolStep - предельное количество ячеек с пропусками подряд, которые заполяются интерполяцией 
real, allocatable :: Awin(:), Asum(:) !  Входной  массив для расчета Cv ряда
real CvWin(100), CvSum(100), PavRate, VolPav, VolPav1, SingleWpav(100,50), SingleWthaw(100,50)
real, allocatable :: Qcrop(:), Qinpol(:)
integer, allocatable ::  Moncrop(:), Yearcrop(:), Daycrop(:)
integer lengthcrop, NumGapsY(100)
...
write(3,'(1x, a150)') 'Date  Qin Qgr Qpol Qpav Qthaw Tin Pin' 
write(4,'(1x, a450)') 'year_number Year1 Year2 datestart datepolend Qy Qmax datemax Qygr Qmmsummer monmmsummer Qmmwin nommwin &
			 ...
			 Wpavs2  Wpavthaw1 Wpavthaw2 Qmaxpavs datemaxpavs Qmaxpavthaw datemaxpavthaw &
			 SumProd DaysPavsSum WinProd DaysThawWin CvWin CvSum CountPavs CountThaws' 

open (5, file=outfileparameters)
write(5,*)'Year polgrad(1) polgrad(2) polkol(1) polkol(2) polkol(3)'
...
do while(l<=w)
  ...
  if(Year1(l)>donep(4)) then !перескачили через 1 год - так и не нашли половодье (из-за неправильных параметров или пропусков)
    ...
    if (NumGapsY(Year1(l)-1)>=30) then
		...
		call Polfinder (Qcrop, lengthcrop, Moncrop, lengthcrop, Yearcrop, lengthcrop, Daycrop, lengthcrop, polmon, 2, &
						FlagGaps, polkol, 3, polgrad, 2, donep, 4, iy(ng-1), ng, iy(ng-hh)+300)
					  
		!print*, 'polfind done';pause
 ...
 if(Year1(l)==donep(4)) then 
  ...
  if (polQsum/(Qin1(l)*polkol(3))<polgrad(2)) then
   ...
   print*, Year1(l),Mon1(l), Day1(l), 'd3', ng 
  end if
  ...
  8787 if (donep(1)==1.and.donep(2)==1.and.donep(3)==1) then !все 3 критерия удовлетворены
    ...
    !print*, ng, l, Year1(l),donep(4), iy(ng-1);pause
...
! Обнуление массивов
 ...
 do ii=1,1000
	  ...
	  Tin(ii)=0
	  FlagsPcr(NF,ii)=0
	  FlagsPlusTemp(NF,ii)=0
	  FlagsMinusTemp(NF,ii)=0
	  Qpav(ii)=0
	  Qthaw(ii)=0
	  CountPavs(ii)=0
	  CountThaws(ii)=0
 ...
 if (YGaps(NF)==1) then !если год с пропусками - не считаем вообще
	...
	if (YGapsRow(NF-1)<1) then
		...
		do pp=iy(NF-1), iy(NF+YGapsRow(NF)-1)
			...
			write(3,'(1x, a15, f10.2, a20, 2f10.2)') dchar, Qin1(pp), '-999 -999 -999 -999', Tin1(pp), Pin1(pp)
		end do
    end if
    cycle
 end if
!назначение массива Qin на данный год
 ...
 do n=1,ny(NF)
	 ...
	 if (n==1.or.n==ny(NF)) then
		  ...
		  dQgr=-(Qgrlast-Qin(n))/Qgrlast*100
		  dQgr1=-(Qgrlast1-Qin(n))/Qgrlast1*100
		  dQgr2=abs((Qgrlast-Qin(n))/(n-nlast+1)/Qgrlast*100)
		  ...
		  if (Qin(n)>Qgrlast) then ! чтобы не рисовал прямых линий
			   if((n-nmax)>prodspada) then  !  длина периода с начала половодья, когда градиенты спада и подъема повышены
					if(dQ>grad.or.dQgr1>kdQgr1.or.dQgr2>grad) then !фильтр для дней сильно после половодья
   						cycle
					end if
			   else
					if(dQ>grad1.or.dQgr2>grad1.or.dQgr1>kdQgr1) then !фильтр для дней сразу же после половодья
						cycle
					end if
			   end if
		  else
			   if(dQ>20*grad.or.dQgr1>20*kdQgr1.or.dQgr2>20*grad) then !фильтр чтобы пропускал косяки с резким выпаданием точки вниз
			   !print*, Day(n), Mon(n),Year(n), dQ
			   !pause
				   if(abs(Qin(n+1)-Qin(n-1))<abs(Qin(n+1)-Qin(n)))then
					!print*, abs(Qin(n+1)-Qin(n-1)), abs(Qin(n+1)-Qin(n))
					!pause
						cycle
				   end if
			   end if
 ...
 do k=1,ny(NF) ! линейная интерполяция грунтовой составляющей в точки без значений
	  ...
	  88  Qygr(NF)=Qygr(NF)+Qgr(k)/ny(NF)
      
  

      ! расчет среднегодового значения
	  Qy(NF)=Qy(NF)+Qin(k)/ny(NF)   
   ...
   if (p<Bend1) then
	 ...
	 if(Flex1>0) then !если мы нашли спад ракоходя
		...
		do pp=LocMax1-HalfStZ, Flex1
			if(FlagsMinusTemp(NF,pp)==1)then !заморозок был по данным температур
				...
				startPol(NF)=Flex1 ! настоящая дата начала половодья - двигаем
				  do rr=Flex1-1,LocMax1, -1 
				     if ((Qin(rr)-Qin(Flex1))>=Qin(nmax)*SignDelta1) then !есть нормальный отрицательный перепад чтобы высчитать экспаненту
				...
				goto 667 !выходим из поиска паводков на подъеме
			end if
		end do	
		!не было замаромков, это либо спад дождевого паводка, либо многопиковой половодье
       ...
       do pp=Bend1-2*HalfSt,LocMax1, 1	
			if(FlagsPcr(NF,pp)==1)then !был дождевой паводок	
	...
	!теперь ищем точку конца паводка
		do pp=Flex2, polend(NF)
			if ((Qin(pp)<Qin(Flex2).and.(min(deltaQ(pp),deltaQ(pp-1))>=(Qin(pp)-Qin(Flex2))/(pp-Flex2))).or.(pp==polend(NF))) then
			   ...
			   if(FlagsPcr(NF,ppp)==1)then !был дождевой паводок
...
!print*, '!!!', polend(NF), 'll', FlagsMinusTemp(NF,polend(NF):ny(NF)), 'ioi', Tin(polend(NF):ny(NF)); pause
...
512 k=polend(NF)
 ...
 do p=1,ny(NF)
	Qpol(p)=Qin(p)-Qgr(p)-Qthaw(p)-Qpav(p)
 	...
 	write(3,'(1x, a15, 7f10.2)') dchar, Qin(p), Qgr(p), Qpol(p), Qpav(p), Qthaw(p), Tin(p), Pin(p) 
 ...
 VolPav1=0
  ...
  do k=polend(NF), SummerEnd(NF)  !лето
	 if(Qin(k)>Qgr(k)) then ! идет паводок                
			if (MarkUpPavs==1) then
				VolPav=VolPav+Qpav(k)*86400/1000000000
				VolPav1=VolPav1+(Qpav(k)+Qgr(k))*86400/1000000000
			else
				MarkUpPavs=1
			end if
	 else
 ...
 !Расчет минимального месячного расхода воды

  Vm(1)=Year(ny(NF)) ! для определения весокосного года\невесокосного года
  ...
  call Mmes (Vm, 1001, MM,4,ny(NF),mome, Mon(1),Mon(ny(NF)), Day(1), Day(ny(NF)))



  krat=30

  call Mkrat (Qin,1000, Day, 1000, Year, 1000, Mon, 1000,ny(NF), krat,mome, Res, 4)
  ...
  call Datechar (Day(i30sn(NF)),Mon(i30sn(NF)),2013, dchar)
  date30s(1)=dchar
  call Datechar (Day(i30sn(NF)+29),Mon(i30sn(NF)+29),2013,dchar)
  date30s(2)=dchar
  call Datechar (Day(i30wn(NF)),Mon(i30wn(NF)),2013,dchar)
  date30w(1)=dchar
  call Datechar (Day(i30wn(NF)+29),Mon(i30wn(NF)+29),2013,dchar)
  date30w(2)=dchar
  ...
  call Datechar (Day(i10sn(NF)),Mon(i10sn(NF)),2013,dchar)
  date10s(1)=dchar
  call Datechar (Day(i30sn(NF)+9),Mon(i30sn(NF)+9),2013,dchar)
  date10s(2)=dchar
  call Datechar (Day(i10wn(NF)),Mon(i10wn(NF)),2013,dchar)
  date10w(1)=dchar
  call Datechar (Day(i10wn(NF)+9),Mon(i10wn(NF)+9),2013,dchar)
  date10w(2)=dchar
  ...
  call Datechar (Day(i5sn(NF)),Mon(i5sn(NF)),2013,dchar)
  date5s(1)=dchar
  call Datechar (Day(i5sn(NF)+4),Mon(i5sn(NF)+4),2013,dchar)
  date5s(2)=dchar
  call Datechar (Day(i5wn(NF)),Mon(i5wn(NF)),2013,dchar)
  date5w(1)=dchar
  call Datechar (Day(i5wn(NF)+4),Mon(i5wn(NF)+4),2013,dchar)
  date5w(2)=dchar


  call Datechar (Day(startPol(NF)), Mon(startPol(NF)),2013, dchar)
  datestart=dchar

  call Datechar (Day(nmax), Mon(nmax),2013, dchar)
  datemax=dchar

  
  call Datechar (Day(polend(NF)), Mon(polend(NF)),2013,dchar)
  datepolend=dchar

  call Datechar (Day(nmaxpavs), Mon(nmaxpavs),2013,dchar)
  datemaxpavs=dchar

  call Datechar (Day(nmaxpavw), Mon(nmaxpavw),2013,dchar)
  datemaxpavw=dchar
...
print*, 'done'
 ...
 write(4,'(1x, i6, 2i10, 2a15, 2f10.2, a15, 6f10.2, 2a15, f10.2, 2a15, f10.2, 2a15, f10.2, 2a15, f10.2, 2a15, f10.2, 2a15, &
             9f13.5, f10.2, a15, f10.2, a15, 4i10, 2f7.3, 2i10)') &
...
subroutine Mmes (vhm,nm,MM,ll,Tot,mome, mo, mok, d1, di) !мин месяц
integer nm, ll, Tot, mome, mo, mok, di, d1
real vhm(nm), MM(ll)
real Qsrm(12), Qsumm(12), SRavn
integer CH(13), m, t, i, h, nomer, s, v1, v2
data CH/0,31,28,31,30,31,30,31,31,30,31,30,31/
...
do t=mo+2,mome
 Do i=1,CH(t)
  m=s+i
  Qsumm(t-1)=Qsumm(t-1)+vhm(m)
 end do
 s=m
 Qsrm(t-1)=Qsumm(t-1)/CH(t)
end do
...
if (di>29) then
 ...
 Do i=1,di
  m=s+i
  Qsumm(mok)=Qsumm(mok)+vhm(m)
 end do
 s=m
 Qsrm(mok)=Qsumm(mok)/di
...
subroutine Mkrat (Qin,qq, Day, qd, Year, qy, Mon, qm,i, krat,mome, Res, qr)

integer qq, qd, qy, qm, i, krat, mome, qr
...
srw=999999
 ...
 Do ii=1,i-(krat+1)
  ...
  if (Year(ii)==Year(1).and.Mon(ii)<mome-1.or.Year(ii)==Year(1).and.Mon(ii)==mome-1.and.Day(ii)<=30-krat/2) then
   s(ii)=1
   do j=1,krat
...
subroutine Polfinder (Qin1, p, Mon1, p1, Year1, p2, Day1, p3, polmon, z, FlagGaps, polkol, y, polgrad, z1, donep, x, iyY, ng, lstart)

integer p, l, p1, p2, p3, z, z1, y, x, Mon1(p1), Year1(p2), Day1(p3), FlagGaps, polmon(z), polkol(y), donep(x), iyY, inipolkol(3), ng
real Qin1(p), polgrad(z1), inipolgrad(2) 
	...
	if(Year1(l)==donep(4)) then 
	  ...
	  if (polQsum/(Qin1(l)*polkol(3))<polgrad(2)) then
		...
		if (donep(1)==1.and.donep(2)==1.and.donep(3)==1) then !все 3 критерия удовлетворены
            ...
            write(5,*) Year1(l), polgrad(1), polgrad(2), polkol(1), polkol(2), polkol(3)
			ng=ng+1 ! число лет
