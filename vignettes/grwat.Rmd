...
- GeoPackage file with polygon of a gauge watershed area (`.gpkg` extension)

The name of the file is not restricted, only the extension and the format. Here is an example:

![](in_gauge.png)

> We use GeoPackage because it is [OGC-standardized](https://www.geopackage.org), fast, modern and powerful spatial data format that does not include any sidecar files (like Esri Shapefile). You can easily convert many spatial data formats to GeoPackage by [QGIS](https://www.qgis.org/) _Export > Save as..._ command in a layer context menu, using `st_read` and then `st_write` functions from [`sf`](https://r-spatial.github.io/sf/) R package, or via [GDAL](http://www.gdal.org) utils if you prefer command line or need batch processing (which can also be automated in R).
